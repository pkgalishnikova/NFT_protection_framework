# experiment 7 - deeper decoder + residual blocks optimizer
...
class Decoder(nn.Module):
    def __init__(self, message_len=48, channels=64, num_blocks=12):
        super().__init__()
        self.stem = ConvBNRelu(3, channels)
        self.res_blocks = nn.Sequential(*[ResBlock(channels) for _ in range(num_blocks)])
        self.head = nn.Sequential(
            ConvBNRelu(channels, message_len),
