...
def differentiable_blur(img, sigma_range=(0.5, 3.0)):
    B, C, H, W = img.shape
...
print(f"   Differentiable blur: sigma=0.5-1.5")
...
for sigma in blur_sigmas:
    ...
    print(f"   sigma={sigma:.1f}: {acc*100:.1f}% accuracy")
...
ax_graph.set_xlabel('Blur Strength (sigma)', fontsize=11, fontweight='bold')
...
After scaling (alpha×tanh):
  alpha = {torch.tanh(enc.alpha).item():.4f}
...
with torch.no_grad():
    ...
    print(f"   After alpha scaling (alpha={alpha_val:.4f}):")
