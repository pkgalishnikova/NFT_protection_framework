...
# 🔧 Helper: Replace tf.image.rgb_to_yuv (removed in TF2)
def rgb_to_yuv(rgb):
...
# 🔧 Helper: Replace tf.contrib.image.transform
def _transform_image(images, transforms_8):
    transforms_8 = tf.cast(transforms_8, tf.float32)
    return tf.raw_ops.ImageProjectiveTransformV3(
        images=images,
        transforms=transforms_8,
        output_shape=[tf.shape(images)[1], tf.shape(images)[2]],
...
class StegaStampEncoder(Layer):
    def __init__(self, height, width):
        super(StegaStampEncoder, self).__init__()
        self.secret_dense = Dense(4096, activation='relu', kernel_initializer='he_normal')  # 64×64×1 = 4096
    ...
    def call(self, inputs):
        ...
        secret = self.secret_dense(secret)
        secret = Reshape((64, 64, 1))(secret)  # 64×64×1 = 4096
        secret_enlarged = secret  # Already 64x64, no upsampling needed!
        secret_enlarged = tf.tile(secret_enlarged, [1, 1, 1, 3])  # Expand to 3 channels
...
def transform_net(encoded_image, args, global_step):
    ...
    ramp_fn = lambda ramp: tf.minimum(tf.cast(global_step, tf.float32) / tf.cast(ramp, tf.float32), 1.)
    ...
    noise = tf.random.normal(shape=tf.shape(encoded_image), mean=0.0, stddev=rnd_noise, dtype=tf.float32)
    encoded_image = encoded_image + noise
    encoded_image = tf.clip_by_value(encoded_image, 0, 1)

    contrast_scale = tf.random.uniform(shape=[tf.shape(encoded_image)[0]], minval=contrast_params[0], maxval=contrast_params[1])
    contrast_scale = tf.reshape(contrast_scale, shape=[tf.shape(encoded_image)[0],1,1,1])
    ...
    encoded_image = tf.reshape(encoded_image, [-1,64,64,3])
    if not args.no_jpeg:
        encoded_image = utils.jpeg_compress_decompress(encoded_image, rounding=utils.round_only_at_0, factor=jpeg_factor, downsample_c=True)

    summaries = [
        tf.compat.v1.summary.scalar('transformer/rnd_bri', rnd_bri),
...
def get_secret_acc(secret_true, secret_pred):
    with tf.compat.v1.variable_scope("acc"):
        secret_pred = tf.round(tf.sigmoid(secret_pred))
        n_bits = tf.shape(secret_pred)[1]
        correct_pred = tf.cast(n_bits, tf.int64) - tf.compat.v1.count_nonzero(
            secret_pred - secret_true, axis=1, dtype=tf.int64
        )

        str_acc = 1.0 - tf.cast(
            tf.compat.v1.count_nonzero(correct_pred - tf.cast(n_bits, tf.int64)), tf.float32
        ) / tf.cast(tf.size(correct_pred), tf.float32)

        bit_acc = tf.cast(tf.reduce_sum(correct_pred), tf.float32) / tf.cast(tf.size(secret_pred), tf.float32)
        return bit_acc, str_acc
...
def build_model(encoder,
    ...
    input_warped = _transform_image(image_input, M[:,1,:])
    mask_warped = _transform_image(tf.ones_like(input_warped), M[:,1,:])
    input_warped += (1 - mask_warped) * image_input

    residual_warped = encoder((secret_input, input_warped))
    encoded_warped = residual_warped + input_warped
    residual = _transform_image(residual_warped, M[:,0,:])

    if borders == 'no_edge':
        encoded_image = image_input + residual
    elif borders == 'black':
        encoded_image = residual_warped + input_warped
        encoded_image = _transform_image(encoded_image, M[:,0,:])
        input_unwarped = _transform_image(input_warped, M[:,0,:])
    elif borders.startswith('random'):
        mask = _transform_image(tf.ones_like(residual), M[:,0,:])
        encoded_image = residual_warped + input_warped
        encoded_image = _transform_image(encoded_image, M[:,0,:])
        input_unwarped = _transform_image(input_warped, M[:,0,:])
        ch = 3 if borders.endswith('rgb') else 1
    ...
    encoded_image_yuv = rgb_to_yuv(encoded_image)
    image_input_yuv = rgb_to_yuv(image_input)
    im_diff = encoded_image_yuv - image_input_yuv
    im_diff += im_diff * tf.expand_dims(falloff_im, axis=[-1])
    yuv_loss_op = tf.reduce_mean(tf.square(im_diff), axis=[0,1,2])
    image_loss_op = tf.tensordot(yuv_loss_op, yuv_scales, axes=1)
