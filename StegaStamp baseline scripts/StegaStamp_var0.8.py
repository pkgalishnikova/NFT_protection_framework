...
def train_model(encoder, decoder, train_loader, num_epochs=15, device='cuda'):
    ...
    for epoch in range(num_epochs):
        ...
        for images, fingerprints, addresses in pbar:
            ...
            # Apply attacks (move to GPU efficiently)
            attacked_images = []
            for i in range(encoded_images.shape[0]):
                img = encoded_images[i].detach()
                img = jpeg_attack(img)
                img = blur_attack(img)
            ...
            loss = loss_image + 15.0 * loss_secret
            ...
            # Calculate metrics
            bit_acc = calculate_bit_accuracy(torch.sigmoid(decoded_fingerprints), fingerprints)
            psnr = calculate_psnr(encoded_images, images)
            ...
            pbar.set_postfix({
                ...
                'PSNR': f'{psnr:.2f} dB',
                'GPU Mem': f'{torch.cuda.memory_allocated()/1024**3:.2f}GB'
            })
...
def evaluate_watermark(encoder, decoder, image_path, ethereum_address, device='cuda', save_images=False):
    ...
    plt.suptitle('StegaStamp Watermarking Pipeline', fontsize=18, fontweight='bold', y=0.98)
    plt.tight_layout()
    plt.show()
    ...
    axes[2].set_title('Difference (10x amplified)', fontsize=14, fontweight='bold')
    axes[2].axis('off')

    plt.suptitle('Watermark Visibility Analysis', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.show()
