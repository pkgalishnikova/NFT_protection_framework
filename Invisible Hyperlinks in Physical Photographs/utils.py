...
def random_blur_kernel(probs, N_blur, sigrange_gauss, sigrange_line, wmin_line):
    ...
    coords = tf.cast(
        tf.stack(tf.meshgrid(tf.range(N_blur), tf.range(N_blur), indexing='ij'), -1),
        tf.float32
    ) - (0.5 * (N - 1))
    manhat = tf.reduce_sum(tf.abs(coords), -1)
    ...
    # nothing, default
    # 🔧 tf.to_float → tf.cast
    vals_nothing = tf.cast(manhat < 0.5, tf.float32)
    ...
    v = tf.convert_to_tensor([tf.cos(theta), tf.sin(theta)], dtype=tf.float32)
    dists = tf.reduce_sum(coords * v, -1)

    sig_line = tf.random.uniform([], sigrange_line[0], sigrange_line[1])
    w_line = tf.random.uniform([], wmin_line, 0.5 * (N - 1) + 0.1)

    # 🔧 tf.to_float → tf.cast
    vals_line = tf.exp(-dists**2 / 2. / sig_line**2) * tf.cast(manhat < w_line, tf.float32)
...
def downsampling_420(image):
    ...
    return (tf.squeeze(y, axis=-1),
            tf.squeeze(cb, axis=-1),
            tf.squeeze(cr, axis=-1))
