...
def diff_jpeg(img, quality=None):
    if quality is None:
        quality = random.randint(50, 90)
    img_01 = (img + 1) / 2
    img_jpeg = kornia.enhance.jpeg_codec_differentiable(img_01,
               torch.tensor([quality], device=img.device).float())
    return img_jpeg * 2 - 1
...
for epoch in range(NUM_EPOCHS):
    ...
    for images in train_loader:
        ...
        if epoch < 15:
            ...
            jpeg_prob = min(0.4, max(0, (epoch - 80) / 100))
            ...
            elif r < blur_prob + rot_prob + jpeg_prob:
                noised = diff_jpeg(encoded_images)
            else:
                noised = encoded_images
