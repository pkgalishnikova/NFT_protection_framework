...
opt_enc_dec = torch.optim.AdamW(list(enc.parameters()) + list(dec.parameters()), lr=1e-3)
opt_disc    = torch.optim.AdamW(disc.parameters(), lr=1e-4)

steps_per_epoch = len(train_loader)
scheduler = torch.optim.lr_scheduler.OneCycleLR(opt_enc_dec, max_lr=2e-3, epochs=140, steps_per_epoch=steps_per_epoch)
...
for epoch in range(NUM_EPOCHS):
    ...
    for images in train_loader:
        ...
        loss = 3.0 * loss_msg + 0.3 * loss_img

        opt_disc.zero_grad()
        real_loss = F.binary_cross_entropy_with_logits(disc(images), torch.ones(B, 1).to(DEVICE))
        fake_loss = F.binary_cross_entropy_with_logits(disc(encoded_images.detach()), torch.zeros(B, 1).to(DEVICE))
        disc_loss = (real_loss + fake_loss) / 2
        disc_loss.backward()
        opt_disc.step()

        opt_enc_dec.zero_grad()
        adv_loss = F.binary_cross_entropy_with_logits(disc(encoded_images), torch.ones(B, 1).to(DEVICE))
        loss = loss + 0.001 * adv_loss
        loss.backward()
        opt_enc_dec.step()
        scheduler.step()
    ...
    if epoch % 10 == 0 or epoch == NUM_EPOCHS - 1:
        ...
        print(f"Эп {epoch:3d} | Loss: {avg_loss:.4f} | Train: {avg_acc:.1%} | "
